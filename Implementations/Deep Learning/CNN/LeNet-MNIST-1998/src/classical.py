...
def _prepare(x_train: np.ndarray, x_test: np.ndarray):
    """Flatten and scale images for classical ML."""
...
def run_svm(x_train, y_train, x_test, y_test):
    print("\n[SVM] Training LinearSVC on 60,000 samples...")
    x_tr, x_te = _prepare(x_train, x_test)

    t0    = time.time()
    model = LinearSVC(max_iter=2000, C=0.1)
...
def run_knn(x_train, y_train, x_test, y_test, k: int = 3):
    print(f"\n[kNN] Training k={k} on 60,000 samples...")
    x_tr, x_te = _prepare(x_train, x_test)
